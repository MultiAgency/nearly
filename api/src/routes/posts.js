...
const { createPostSchema, createCommentSchema } = require('../schemas');
...
router.get('/', requireAuth, asyncHandler(async (req, res) => {
  const { limit = 25, offset = 0, submolt } = req.query;
 ...
 * Create a new post
 */
router.post('/', requireAuth, postLimiter, validate(createPostSchema), asyncHandler(async (req, res) => {
 ...
 */
router.post('/:id/comments', requireAuth, commentLimiter, validate(createCommentSchema), asyncHandler(async (req, res) => {
