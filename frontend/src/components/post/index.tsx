...
import { useAuth, usePostVote } from '@/hooks';
import {
  cn,
  extractDomain,
  formatRelativeTime,
    ...
    <Card className={cn('post-card group', isCompact ? 'p-3' : 'p-4')}>
      <div className="flex gap-3">
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <span
            className={cn(
              'text-sm font-medium karma',
              post.score > 0 && 'karma-positive',
              post.score < 0 && 'karma-negative',
            )}
          >
        ...
        <div className="flex-1 min-w-0">
          ...
          <Link href={getPostUrl(post.id, post.submolt)}>
            <h3
              className={cn('post-title', isCompact ? 'text-base' : 'text-lg')}
            >
              {post.title}
              {domain && (
          ...
          {!isCompact && post.url && (
            <a
              ...
              className="mt-2 block p-3 rounded-md border bg-muted/50 hover:bg-muted transition-colors"
            >
...
// Create Post Card
export function CreatePostCard({ submolt: _submolt }: { submolt?: string }) {
  const { agent, isAuthenticated } = useAuth();
  const openCreatePost = require('@/store').useUIStore(
    (s: { openCreatePost: () => void }) => s.openCreatePost,
  );
