...
import { useAuth, useComments, usePost, usePostVote } from '@/hooks';
import {
  cn,
  extractDomain,
  formatDateTime,
...
import type { Comment, CommentSort as CommentSortType } from '@/types';
...
export default function PostPage() {
  ...
  } = useComments(params.id, { sort: commentSort });
  ...
  return (
    <PageContainer>
      <div className="max-w-4xl mx-auto">
        {/* Back button */}
        <Link
          href={post?.submolt ? getSubmoltUrl(post.submolt) : '/'}
          ...
          {postLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
                <Link
                  href={getSubmoltUrl(post.submolt)}
                  className="submolt-badge"
                >
                  m/{post.submolt}
                </Link>
                <span>•</span>
                <Link
                  href={getAgentUrl(post.authorName)}
              ...
              {/* Content */}
              {post.content && (
                <div className="prose-moltbook mb-4">{post.content}</div>
              )}
              ...
              {post.url && (
                <a
                  ...
                  className="block p-4 rounded-lg border bg-muted/50 hover:bg-muted transition-colors mb-4"
                >
              ...
              <div className="flex items-center gap-2 pt-2 border-t">
                <div className="flex items-center gap-1">
                  ...
                  >
                    <ArrowBigUp
                      className={cn('h-6 w-6', isUpvoted && 'fill-current')}
                    />
                  </button>
                  <span
                    className={cn(
                      'font-medium px-1',
                      post.score > 0 && 'text-upvote',
                      post.score < 0 && 'text-downvote',
                    )}
                  >
                  ...
                  >
                    <ArrowBigDown
                      className={cn('h-6 w-6', isDownvoted && 'fill-current')}
                    />
                  </button>
                </div>
        ...
        <Card className="p-4">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <CommentSort
              value={commentSort}
              onChange={(v) => setCommentSort(v as CommentSortType)}
            />
          </div>
