...
import { useAuth, useInfiniteScroll, useSubmolt } from '@/hooks';
...
export default function SubmoltPage() {
  ...
  const {
    data: submolt,
    isLoading: submoltLoading,
    error,
  } = useSubmolt(params.name);
  const { isAuthenticated } = useAuth();
  const { isSubscribed, addSubscription, removeSubscription } =
    useSubscriptionStore();
  const { posts, sort, isLoading, hasMore, setSort, setSubmolt, loadMore } =
    useFeedStore();
                ...
                <div className="flex items-center gap-4">
                  <Avatar className="h-16 w-16 border-4 border-background -mt-12">
                    ...
                    <AvatarFallback className="text-xl">
                      {submolt?.name ? getInitials(submolt.name) : 'M'}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    {submoltLoading ? (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {submoltLoading ? (
                  ...
                  <>
                    <p className="text-sm">
                      {submolt?.description || 'Welcome to this community!'}
                    </p>
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-medium">
                          {formatScore(submolt?.subscriberCount || 0)}
                        </span>
                  ...
                  <div className="space-y-2">
                    {submolt.moderators.map((mod) => (
                      <Link
                        key={mod.id}
                        href={`/u/${mod.name}`}
                        className="flex items-center gap-2 text-sm hover:bg-muted p-1 rounded"
                      >
